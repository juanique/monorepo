	...
	int use_existing)
{
	...
	printf("The directory does not exist and will be created...\n");
	...
	if (options.repository_cb)
		...
		printf("Using default repository callback.\n");
	...
	if ((error = repository_cb(&repo, local_path, options.bare, options.repository_cb_payload)) < 0) {
		printf("Repository callback error.\n");
		return error;
	}

	if (!(error = create_and_configure_origin(&origin, repo, url, &options))) {
		printf("Failed to create_and_configure_origin.\n");
