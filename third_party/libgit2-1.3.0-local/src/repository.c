	...
	git_repository_init_options *opts)
{
	...
	if (!error) {
		printf("before git_path_prettify_dir\n");
	...
	printf("done repo_init_directories\n");
	return error;
}
	...
	git_repository_init_options *opts)
{
	...
	printf("Before repo_init_directories\n");
	if ((error = repo_init_directories(&repo_path, &wd_path, given_repo, opts)) < 0)
		goto out;

	wd = (opts->flags & GIT_REPOSITORY_INIT_BARE) ? NULL : git_buf_cstr(&wd_path);

	printf("Before is_valid_repository_path\n");
	...
	if (is_valid) {
		...
		printf("Before repo_init_config\n");
	...
	printf("Before git_repository_open\n");
	if ((error = git_repository_open(out, repo_path.ptr)) < 0)
		goto out;

	printf("Before repo_init_create_origin\n");
	if (opts->origin_url &&
